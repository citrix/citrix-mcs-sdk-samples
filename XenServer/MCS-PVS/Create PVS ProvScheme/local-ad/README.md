...
This section explains how to create a **Citrix Provisioning Services (PVS) Provisioning Scheme (ProvScheme)** in **Citrix Virtual Apps and Desktops (CVAD)** for **local Active Directory-joined** catalogs hosted on **XenServer**.
...
## 1. Requirements for XenServer

For **Citrix Virtual Apps and Desktops (CVAD) / on-premises** and **Citrix DaaS (Citrix Cloud)** deployments on **XenServer**, the minimum supported versions are **CVAD 2402 and later** and **Citrix Provisioning (PVS) 2402 and later**.
...
3. Create a XenServer hosting connection and hosting unit
4. Run the PVS Configuration Wizard to join PVS farm to CVAD/DaaS
5. Retrieve PVS site details
6. Prepare the machine profile (template VM or snapshot) used for catalog hardware settings
...
- Ensure:
  ...
  - Network connectivity to:
    ...
    - XenServer management and VM networks where targets will run
...
## 2.3 Create a XenServer Hosting Connection and Hosting Unit

Set up a **XenServer hosting connection and hosting unit** in Studio / Web Studio, just as you would for an MCS catalog in Citrix Virtual Apps and Desktops (CVAD). This connection will allow the provisioning of virtual machines on XenServer.

Use the XenServer-specific starter scripts in this repository for those prerequisites:
...
For XenServer, the PVS provisioning scheme requires a **machine profile** so MCS can derive the VM hardware configuration (CPU, memory, NIC layout, etc.) for the catalog.
...
## 2.7 Example `New-ProvScheme` Pattern for XenServer PVS
...
For the complete example, refer to [`Create-PvsProvScheme-LocalAD.ps1`](./Create-PvsProvScheme-LocalAD.ps1).