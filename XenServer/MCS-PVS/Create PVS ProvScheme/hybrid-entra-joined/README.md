...
This folder contains an example PowerShell script for creating a **Citrix Provisioning Services (PVS)**-backed catalog in **Citrix Virtual Apps and Desktops (CVAD)** / **Citrix DaaS** where machines are **Hybrid Entra joined** and hosted on **XenServer**.

- Script: [`Create-PvsProvScheme-HybridEntraJoined.ps1`](./Create-PvsProvScheme-HybridEntraJoined.ps1)
- Base flow (PVS setup, vDisk, XenServer hosting connection, PVS → CVAD/DaaS registration, etc.) is the same as [`Traditional AD`](../traditional-ad/README.md)
...
For **CVAD / on-premises** and **Citrix DaaS (Citrix Cloud)** deployments on **XenServer**, the minimum supported versions are **CVAD / DaaS 2402 and later** and **Citrix Provisioning (PVS) 2402 and later**.
...
- **Hybrid join & sync prerequisites**
  ...
  - Ensure Azure AD / Microsoft Entra endpoints are reachable from the XenServer-hosted VMs (network, proxy, firewall).
