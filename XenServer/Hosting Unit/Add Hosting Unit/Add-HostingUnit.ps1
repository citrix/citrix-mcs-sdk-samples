...
param(
    ...
    [switch] $UseLocalStorageCaching = $false,
    [string] $GpuTypePath = $null
)
...
# If UseLocalStorageCaching is specified, configure the UseLocalStorageCaching.
if ($UseLocalStorageCaching) { $newItemParameters['UseLocalStorageCaching'] = $UseLocalStorageCaching }

# If GpuTypePath is specified, configure the GpuTypePath.
if ($GpuTypePath) { $newItemParameters['GpuTypePath'] = $GpuTypePath }
