...
param(
    ...
    [guid] $ZoneUid,
    [switch] $UseWriteBackCache = $false, # XenServer Feature
    [int] $WriteBackCacheDiskSize, # XenServer Feature
    [int] $WriteBackCacheMemorySize, # XenServer Feature
    [string] $WriteBackCacheDriveLetter # XenServer Feature
)
...
$newProvSchemeParameters = @{
    ...
    UseWriteBackCache       = $UseWriteBackCache
