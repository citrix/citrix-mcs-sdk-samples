<#
.SYNOPSIS
    Adds a VirtualMachine in an MCS catalog. Applicable for Citrix DaaS and on-prem.
.DESCRIPTION
    New-ProvVM.ps1 helps to add the VM to an existing MCS catalog.
...
##########################################################################################
# Step 1: Add an Active Directory(AD) computer account in the specified identity pool. #
##########################################################################################
...
############################################################################
# Step 2: Create virtual machines in the provided ProvisioningSchemeName. #
############################################################################
...
#################################################################################
# Step 3: Lock virtual machines in the provisioning scheme with a specific tag. #
#################################################################################

$newProvVms = Get-ProvVM -ProvisioningSchemeName $provisioningSchemeName
$newProvVms | ForEach-Object { Lock-ProvVM -ProvisioningSchemeName $provisioningSchemeName -Tag $lockTag -VMID $_.VMId }

##############################################################
# Step 4: Create Broker machines for the above created VMs. #
##############################################################
