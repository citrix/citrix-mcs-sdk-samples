...
# An assgined Azure AD security group name to join.
$securtiyGroupName = "DemoGroupName"
...
{
    throw "Could not find the Azure AD security group (Azure AD Security Group Name): $($securityGroupMemberName). Verify the security group name exists."
}

######################################################################################
# Step 8: Get the assigned Azure AD security group to verfiy if it's ready to join.  #
######################################################################################
$securtiyGroup = Get-AcctAzureADSecurityGroup  -AccessToken $token.Token -MaxRecordCount 300 -Name $securtiyGroupName
if($null -eq $securtiyGroup.ObjectId)
{
    throw "Could not find the Azure AD security group (Azure AD Security Group Name): $($securtiyGroupNameName). Verify the security group name exists."
}

#########################################################
# Step 9: Join an assigned security group as an member  #
#########################################################
Add-AcctAzureADSecurityGroupMember -AccessToken $token.Token -GroupId $securtiyGroup.ObjectId -RefGroupId $securtiyGroupMember.ObjectId

$members = Get-AcctAzureADSecurityGroupMember -AccessToken $token.Token -GroupId $securtiyGroup.ObjectId
$found = $false
foreach($item in $members) {
    if($item.Name -eq $securityGroupMemberName) {
        $found = $true
        break
    }
}
if(-not $found) {
    throw "Could not find the member Azure AD security group (Azure AD Security Group Name): $($securityGroupMemberName) from $($securtiyGroupName)."
}