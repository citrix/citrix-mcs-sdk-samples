...
This folder contains an example PowerShell script for creating a **Citrix Provisioning Services (PVS)**–backed catalog in **Citrix Virtual Apps and Desktops (CVAD)** / **Citrix DaaS** where machines are **Hybrid Entra joined** and hosted in **Azure**.

- Script: [`Create-PvsProvScheme-HybridEntraJoined.ps1`](./Create-PvsProvScheme-HybridEntraJoined.ps1)
- Base flow (PVS setup, vDisk, Azure hosting connection, PVS → CVAD/DaaS registration, etc.) is the same as [`Traditional AD`](../traditional-ad/README.md)
...
For **CVAD / on‑premises** and **Citrix DaaS (Citrix Cloud)** deployments on **Azure**, the minimum supported versions are **CVAD / DaaS 2402 and later** and **Citrix Provisioning (PVS) 2402 and later**.

> Hybrid Entra joined catalogs:  
> https://docs.citrix.com/en-us/provisioning/2402-ltsr/configure/create-hybrid-joined-catalogs  
> PVS catalog creation and Azure limitations:  
...
- **Hybrid join & sync prerequisites**  
  ...
  - Ensure Azure AD / Microsoft Entra endpoints are reachable from the Azure‑hosted VMs (network, proxy, firewall).  
