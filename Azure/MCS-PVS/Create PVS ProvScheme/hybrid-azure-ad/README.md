...
This folder contains an example PowerShell script for creating a **Citrix Provisioning Services (PVS)**–backed catalog in **Citrix Virtual Apps and Desktops (CVAD)** / **Citrix DaaS** where machines are **Hybrid Azure AD joined** and hosted in **Azure**.

- Script: [`Create-PvsProvScheme-HybridAzureAD.ps1`](./Create-PvsProvScheme-HybridAzureAD.ps1)
- Base flow (PVS setup, vDisk, Azure hosting connection, PVS → CVAD/DaaS registration, etc.) is the same as [`Local AD`](../local-ad/README.md)
