<#
.SYNOPSIS
    Creates an MCS catalog using a Dsv6 VM size (NVMe-only) with DiskControllerType unspecified in the machine profile.
    Result: VMs are provisioned with NVMe storage (default for NVMe-only SKUs). Applicable for Citrix DaaS and on-prem.
.DESCRIPTION
    Create-DsV6-NVMeDefault.ps1 demonstrates creating an MCS catalog where the service offering is from the
    Dsv6 series (NVMe-only — Azure v6 and later VM generations support NVMe exclusively) and the machine profile VM
