...
if(($hostingUnits | Measure-Object).Count -gt 0)
{
    ...
    $hostingUnits.HostingUnitName | ForEach-Object {
        ...
        try { Remove-Item -LiteralPath ("XDHyp:\HostingUnits\"+$_) -Force -ErrorAction Stop }
        catch {
            Write-Error $_
            exit
        }
...
try { & Remove-Item -LiteralPath $connectionPath  -ErrorAction Stop}
...
try { Remove-BrokerHypervisorConnection -Name $ConnectionName  -ErrorAction Stop}
catch {
    Write-Error $_
    exit
}